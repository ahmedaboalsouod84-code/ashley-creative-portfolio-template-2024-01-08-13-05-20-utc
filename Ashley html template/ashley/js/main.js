...
$(function () {
    ...
    ***************************/
    ...
    // Logo swap helper: use icons/2.png on light backgrounds, icons/1.png otherwise.
    function milIsLightColor(rgb) {
        var match = rgb && rgb.match(/rgba?\(([^)]+)\)/);
    ...
    function milSwapLogos() {
        var lightLogo = 'img/icons/2.png';
        var darkLogo = 'img/icons/1.png';
        var header = document.querySelector('.mil-frame-top') || document.querySelector('.mil-menu-frame .mil-frame-top');
        var bg = header ? window.getComputedStyle(header).backgroundColor : null;
        if (!bg || bg === 'rgba(0, 0, 0, 0)' || bg === 'transparent') {
            bg = window.getComputedStyle(document.body).backgroundColor;
        }
        var useLight = bg ? milIsLightColor(bg) : false;
        var chosen = useLight ? lightLogo : darkLogo;
        document.querySelectorAll('.mil-logo img').forEach(function (img) {
            img.src = chosen;
        });
    }
